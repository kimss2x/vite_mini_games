...
const DominoCanvas: React.FC = () => {
  ...
  const start = useCallback(() => {
    if (running) return;
    const idxs = [START_INDEX, ...Array.from(placements).sort((a, b) => a - b)];
  ...
  useEffect(() => {
      ...
      dominos.forEach((d, i) => {
        if (d.falling && !d.fallen) {
          ...
          if (d.angle >= Math.PI / 2) {
            ...
            d.fallen = true;
            setScore((s) => s + 1);
            if (d.index === GOAL_INDEX - 1) {
              setGoalHit(true);
              setRunning(false);
            } else if (
              i + 1 < dominos.length &&
              dominos[i + 1].index === d.index + 1
            ) {
    ...
    const draw = () => {
      ...
      // draw goal
      ctx.fillStyle = goalHit ? '#0f0' : '#f55';
      ctx.fillRect(slotX(GOAL_INDEX), groundY, DOMINO_WIDTH, DOMINO_HEIGHT);
      ...
      if (running) {
        dominosRef.current.forEach((d) => drawDomino(d.x, d.angle, '#f2f2f2'));
      } else {
        // draw placements
