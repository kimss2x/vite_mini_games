...
// allow enough pieces to fill every slot between start and goal
const MAX_PIECES = POSITIONS - 2; // player placeable dominos
    ...
    const update = () => {
      ...
      dominos.forEach((d, i) => {
        if (d.falling && !d.fallen) {
          ...
          if (next && next.index === d.index + 1 && !next.falling) {
            const tipX =
              d.x + DOMINO_WIDTH / 2 + Math.sin(d.angle) * DOMINO_HEIGHT;
            if (tipX >= next.x) next.falling = true;
          }
