...
const BASE = COLS; // initial top border
...
export default function QixCanvas() {
  ...
  const captureArea = () => {
    ...
    trailRef.current = [];
    setScore((s) => {
      const total = s + gained + BASE;
      if (total >= TARGET) setState('clear');
      return s + gained;
    });
  };
  ...
  return (
    <GameLayout
      ...
      topInfo={
        <div>
          Score: {score} / {TOTAL} (
          {Math.floor(((score + BASE) / TOTAL) * 100)}%)
          {state === 'over' && ' - Game Over'}
          {state === 'clear' && ' - Clear'}
        </div>
      }
      bottomInfo={<div>WASD/Arrow 이동, R=Reset, 75% 점령 시 클리어</div>}
    >
      <GameCanvas
        ref={canvasRef}
        gameTitle="Qix"
