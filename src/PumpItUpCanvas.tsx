...
type Note = { lane: number; y: number };
...
const PumpItUpCanvas: React.FC = () => {
    ...
    const loop = (time: number) => {
      ...
      if (state === 'playing') {
        ...
        if (spawnRef.current > SPAWN_INTERVAL) {
          ...
          notesRef.current.push({ lane: Math.floor(Math.random() * 4), y: -20 });
        }
      }

      for (const note of notesRef.current) {
        if (state === 'playing') note.y += NOTE_SPEED * (delta / 1000);
        ctx.fillStyle = '#0bf';
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (['arrowleft', 'arrowdown', 'arrowup', 'arrowright', 'r'].includes(key)) {
        e.preventDefault();
        e.stopPropagation();
      }
      if (key === 'r') {
      ...
      else if (key === 'arrowup') lane = 2;
      else if (key === 'arrowright') lane = 3;
      if (lane === null) return;
      const note = notesRef.current.find(
        (n) => n.lane === lane && Math.abs(n.y - TARGET_Y) < HIT_WINDOW
      );
      if (note) {
        notesRef.current.splice(notesRef.current.indexOf(note), 1);
        setScore((s) => s + 1);
      }
    },
    [resetGame, state]
  ...
  return (
    <GameLayout
      ...
      bottomInfo={<div>←↓↑→ : Hit • R : Reset</div>}
    >
