...
const PuyoPuyoCanvas: React.FC = () => {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (key === 'arrowleft' || key === 'a') {
        if (canMove(-1, 0, 0)) pairRef.current.x--;
      } else if (key === 'arrowright' || key === 'd') {
        if (canMove(1, 0, 0)) pairRef.current.x++;
      } else if (key === 'arrowdown' || key === 's') {
        if (canMove(0, 1, 0)) pairRef.current.y++;
      } else if (key === 'arrowup' || key === 'x') {
        if (canMove(0, 0, 1)) pairRef.current.rotation = (pairRef.current.rotation + 1) % 4;
      } else if (key === 'z') {
        if (canMove(0, 0, -1)) pairRef.current.rotation = (pairRef.current.rotation + 3) % 4;
      } else if (key === 'r') {
        resetGame();
      }
      draw();
  ...
  }, [draw, resetGame, state]);

  return (
    <GameLayout
      title="🍡 Puyo Puyo"
      topInfo={<div>Score: {score}</div>}
      bottomInfo={<div>좌/우/아래 이동, 위/ X/Z 회전, R=Reset</div>}
    >
      <GameCanvas
        ref={canvasRef}
        gameTitle="Puyo Puyo"
